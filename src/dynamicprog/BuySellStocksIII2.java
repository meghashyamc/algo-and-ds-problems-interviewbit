...
public class BuySellStocksIII2 {
	...
	public int maxProfit(final List<Integer> A) {
		...
		// max profit in 2 transactions = max of (max profit with one transaction from 0
		// to i,
	...
	// returns max profit if only one transaction can happen from lo to hi indices
	// in list A
	private int getMaxOneTrans(int lo, int hi, List<Integer> A) {

		if ((lo >= A.size()) || (hi >= A.size()))
			return 0;
