...
public class MedianOfTwoArrays1 {
	...
	static int kth(List<Integer> a, List<Integer> b, int m, int n, int k, int st1, int st2) {
		...
		if (k == 1) {
			return (a.get(st1) < b.get(st2) ? a.get(st1) : b.get(st2));
		}
		...
		if (curr - 1 >= m - st1) {
			...
			if (a.get(m - 1) < b.get(st2 + curr - 1)) {
				...
				return kth(a, b, m, n, k - curr, st1, st2 + curr);
			}
		}
		...
		if (curr - 1 >= n - st2) {
			...
			if (b.get(n - 1) < a.get(st1 + curr - 1)) {
				...
				// remove the smallest curr elements of array 2 (starting from st2)
				// from consideration and search for k - currth element
				return kth(a, b, m, n, k - curr, st1 + curr, st2);
			}
		} else
