...
public class CanJumpToEnd2 {

        public int canJump(ArrayList<Integer> A) {

            if (A.size() <= 1) return 1;

            //represents the max index that can be reached from
            // any of the indices considered so far
            int max = 0;

            for (int i = 0; i < A.size(); i++) {
                //if the value at current index is 0 and max is less than
                // current index, there's no way to move forward
                if ((max <= i) && A.get(i).equals(0))
                    return 0;
                ...
                //if max >= last index, then we can reach the end of the array
                if (max >= A.size() - 1) return 1;
    ...
    public static void main(String[] args) {
        ...
        CanJumpToEnd2 canJumpToEnd2 = new CanJumpToEnd2();

        System.out.println(canJumpToEnd2.canJump(a));
